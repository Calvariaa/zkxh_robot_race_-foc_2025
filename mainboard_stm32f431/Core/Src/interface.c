...
void get_sensor(void) {
  ...
  for (uint8_t x = 1; x < 127; x++) {
    for (uint8_t y = 2; y < 8; y++) {
      if (gray_forward_value[(x - 1) / 7])
        ssd1306_SetColor(White);
      else
        ssd1306_SetColor(Black);

      ssd1306_DrawPixel(x, 10 + y);
    }
  }
  for (uint8_t x = 34; x < 94; x++) {
    for (uint8_t y = 2; y < 8; y++) {
      if (gray_backward_value[(x - 34) / 10])
        ssd1306_SetColor(White);
      else
        ssd1306_SetColor(Black);

      ssd1306_DrawPixel(x, 50 + y);
    }
  }
  ssd1306_SetColor(White);

  screen_show_float(0, 3, speed_struct.left, 5, 3);
